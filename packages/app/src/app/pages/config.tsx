...
export default function ConfigView(props: ConfigViewProps) {
  ...
  const openworkStatusLabel = createMemo(() => {
    switch (props.openworkServerStatus) {
      case "connected":
        return translate("config.status.connected");
      case "limited":
        return translate("config.status.limited");
      default:
        return translate("config.status.disconnected");
    }
  });
  ...
  const reloadButtonLabel = createMemo(() =>
    props.reloadBusy ? translate("config.reload.loading") : translate("config.reload.action"),
  );
  ...
  const hostStatusLabel = createMemo(() => {
    if (!hostInfo()?.running) return translate("config.host.offline");
    return translate("config.host.available");
  });
  ...
  return (
    <section class="space-y-6">
      <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-2">
        <div class="text-sm font-medium text-gray-12">{translate("config.workspace.title")}</div>
        <div class="text-xs text-gray-10">
          {translate("config.workspace.description")}
        </div>
      <Show when={props.openworkServerWorkspaceId}>
        <div class="text-[11px] text-gray-7 font-mono truncate">
          {translate("config.workspace.label").replace("{id}", props.openworkServerWorkspaceId as string)}
        </div>
      </Show>
      </div>
      ...
      <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-4">
        <div>
          <div class="text-sm font-medium text-gray-12">{translate("config.reload.title")}</div>
          <div class="text-xs text-gray-10">{translate("config.reload.subtitle")}</div>
        </div>

        <div class="flex items-center justify-between bg-gray-1 p-3 rounded-xl border border-gray-6 gap-3">
          <div class="min-w-0 space-y-1">
            <div class="text-sm text-gray-12">{translate("config.reload.now")}</div>
            <div class="text-xs text-gray-7">{translate("config.reload.now_hint")}</div>
            <Show when={props.anyActiveRuns}>
              <div class="text-[11px] text-amber-11">
                {translate("config.reload.active_runs_warning")}
              </div>
            </Show>
        ...
        <div class="flex items-center justify-between bg-gray-1 p-3 rounded-xl border border-gray-6 gap-3">
          <div class="min-w-0 space-y-1">
            <div class="text-sm text-gray-12">{translate("config.auto_reload.title")}</div>
            <div class="text-xs text-gray-7">{translate("config.auto_reload.subtitle")}</div>
            <Show when={!props.workspaceAutoReloadAvailable}>
              <div class="text-[11px] text-gray-9">{translate("config.auto_reload.desktop_only")}</div>
            </Show>
          </div>
          <Button
            variant="outline"
          ...
          >
            {props.workspaceAutoReloadEnabled
              ? translate("config.toggle.on")
              : translate("config.toggle.off")}
          </Button>
        </div>

        <div class="flex items-center justify-between bg-gray-1 p-3 rounded-xl border border-gray-6 gap-3">
          <div class="min-w-0 space-y-1">
            <div class="text-sm text-gray-12">{translate("config.auto_reload_resume.title")}</div>
            <div class="text-xs text-gray-7">
              {translate("config.auto_reload_resume.subtitle")}
            </div>
          </div>
          <Button
            variant="outline"
            ...
            title={props.workspaceAutoReloadEnabled ? "" : translate("config.auto_reload_resume.disabled_hint")}
          >
            {props.workspaceAutoReloadResumeEnabled
              ? translate("config.toggle.on")
              : translate("config.toggle.off")}
          </Button>
        </div>
      ...
      <Show when={props.developerMode}>
        <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-3">
          <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <div class="text-sm font-medium text-gray-12">{translate("config.diagnostics.title")}</div>
              <div class="text-xs text-gray-10">{translate("config.diagnostics.subtitle")}</div>
            </div>
            <Button
              variant="secondary"
              class="text-xs h-8 py-0 px-3 shrink-0"
            ...
            >
              {copyingField() === "debug-bundle"
                ? translate("config.copy.copied")
                : translate("config.copy.copy")}
            </Button>
          </div>
      ...
      <Show when={hostInfo()}>
        <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-4">
          <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <div class="text-sm font-medium text-gray-12">{translate("config.sharing.title")}</div>
              <div class="text-xs text-gray-10">{translate("config.sharing.subtitle")}</div>
            </div>
          ...
          <div class="grid gap-3">
            <div class="flex items-center justify-between bg-gray-1 p-3 rounded-xl border border-gray-6 gap-3">
              <div class="min-w-0">
                <div class="text-xs font-medium text-gray-11">{translate("config.sharing.server_url")}</div>
                <div class="text-xs text-gray-7 font-mono truncate">
                  {hostConnectUrl() || translate("config.sharing.starting_server")}
                </div>
                <Show when={hostConnectUrl()}>
                  <div class="text-[11px] text-gray-8 mt-1">
                    {hostConnectUrlUsesMdns()
                      ? translate("config.sharing.mdns_hint")
                      : translate("config.sharing.lan_hint")}
                  </div>
                </Show>
              </div>
                <Button
            ...
            <div class="flex items-center justify-between bg-gray-1 p-3 rounded-xl border border-gray-6 gap-3">
              <div class="min-w-0">
                <div class="text-xs font-medium text-gray-11">{translate("config.sharing.access_token")}</div>
                <div class="text-xs text-gray-7 font-mono truncate">
                  {clientTokenVisible()
                    ? hostInfo()?.clientToken || translate("config.token.missing")
                    : hostInfo()?.clientToken
                      ? translate("config.token.masked")
                      : translate("config.token.missing")}
                </div>
                <div class="text-[11px] text-gray-8 mt-1">{translate("config.sharing.access_token_hint")}</div>
              </div>
              <div class="flex items-center gap-2 shrink-0">
                ...
                >
                  {clientTokenVisible()
                    ? translate("config.token.hide")
                    : translate("config.token.show")}
                </Button>
                <Button
                  variant="outline"
                  class="text-xs h-8 py-0 px-3"
            ...
            <div class="flex items-center justify-between bg-gray-1 p-3 rounded-xl border border-gray-6 gap-3">
              <div class="min-w-0">
                <div class="text-xs font-medium text-gray-11">{translate("config.sharing.server_token")}</div>
                <div class="text-xs text-gray-7 font-mono truncate">
                  {hostTokenVisible()
                    ? hostInfo()?.hostToken || translate("config.token.missing")
                    : hostInfo()?.hostToken
                      ? translate("config.token.masked")
                      : translate("config.token.missing")}
                </div>
                <div class="text-[11px] text-gray-8 mt-1">{translate("config.sharing.server_token_hint")}</div>
              </div>
              <div class="flex items-center gap-2 shrink-0">
                <Button
                  variant="outline"
          ...
          <div class="text-xs text-gray-9">
            {translate("config.sharing.share_hint").replace(
              "{share}",
              translate("config.share_action"),
            )}
          </div>
        </div>
      </Show>
      ...
      <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-4">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            <div class="text-sm font-medium text-gray-12">{translate("config.server.title")}</div>
            <div class="text-xs text-gray-10">
              {translate("config.server.subtitle")}
            </div>
          </div>
        ...
        <div class="grid gap-3">
          <TextInput
            label={translate("config.server.url_label")}
            value={openworkUrl()}
            onInput={(event) => setOpenworkUrl(event.currentTarget.value)}
            placeholder={translate("config.server.url_placeholder")}
            hint={translate("config.server.url_hint")}
            disabled={props.busy}
          />

          <label class="block">
            <div class="mb-1 text-xs font-medium text-gray-11">{translate("config.server.token_label")}</div>
            <div class="flex items-center gap-2">
              <input
                type={openworkTokenVisible() ? "text" : "password"}
                value={openworkToken()}
                onInput={(event) => setOpenworkToken(event.currentTarget.value)}
                placeholder={translate("config.server.token_placeholder")}
                disabled={props.busy}
            ...
            <div class="mt-1 text-xs text-gray-10">{translate("config.server.token_hint")}</div>
          </label>
        </div>
        ...
        <div class="space-y-1">
          ...
          <div class="text-[11px] text-gray-7 font-mono truncate">
            {translate("config.server.resolved_url").replace(
              "{url}",
              resolvedWorkspaceUrl() || translate("config.server.not_set"),
            )}
          </div>
          <div class="text-[11px] text-gray-8 font-mono truncate">
            {translate("config.server.workspace_id").replace(
              "{id}",
              resolvedWorkspaceId() || translate("config.server.unavailable"),
            )}
          </div>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            onClick={async () => {
              ...
              try {
                ...
                setOpenworkTestMessage(
                  ok
                    ? translate("config.server.test_success")
                    : translate("config.server.test_failed_hint"),
                );
              } catch (error) {
                const message = error instanceof Error ? error.message : translate("config.server.test_failed");
          ...
          >
            {openworkTestState() === "testing"
              ? translate("config.server.testing")
              : translate("config.server.test")}
          </Button>
          <Button
            variant="outline"
        ...
        <Show when={openworkTestState() !== "idle"}>
          ...
          >
            {openworkTestState() === "testing"
              ? translate("config.server.testing_status")
              : openworkTestMessage() ?? translate("config.server.status_updated")}
          </div>
        </Show>

        <Show when={props.openworkServerStatus !== "connected"}>
          <div class="text-xs text-gray-9">{translate("config.server.sync_required")}</div>
        </Show>
      </div>

      <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-2">
        <div class="text-sm font-medium text-gray-12">{translate("config.messaging.title")}</div>
        <div class="text-xs text-gray-10">
          {translate("config.messaging.subtitle").replace(
            "{identities}",
            translate("config.messaging.identities_tab"),
          )}
        </div>
      </div>
