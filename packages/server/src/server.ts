...
function resolveOpenCodeRouterProxyPolicy(
  ...
  if (upper === "GET") {
    ...
    if (
      normalized === "/owpenbot/identities/telegram"
      || normalized === "/owpenbot/identities/slack"
      || normalized === "/owpenbot/identities/wecom"
    ) {
      return { auth: "client", requiredScope: "collaborator" };
    }
  }
...
function createRoutes(config: ServerConfig, approvals: ApprovalService, tokens: TokenService): Route[] {
  ...
  addRoute(routes, "GET", "/workspace/:id/owpenbot/identities/wecom", "client", async (ctx) => {
    requireClientScope(ctx, "collaborator");
    const workspace = await resolveWorkspace(config, ctx.params.id);
    ...
    const apply = await tryFetchOwpenbotHealth("GET", "/identities/wecom", {
      port,
      requestHost,
      timeoutMs: 2_000,
    });
      ...
      if (Array.isArray(rawItems)) {
        const items = rawItems
          ...
          .map((entry) => {
            ...
            const enabled = entry.enabled === undefined ? true : entry.enabled === true || entry.enabled === "true";
            const running = entry.running === true || entry.running === "true";
            const mode = entry.mode === "app" ? "app" : "ai-bot";
            return { id, enabled, running, mode };
          })
          .filter((item) => item.id === workspaceIdentityId);
        return jsonResponse({ ...payload, items });
      }
    ...
    const channels = ensurePlainObject(current.channels);
    const wecom = ensurePlainObject(channels.wecom);
    const appsRaw = (wecom as any).apps;
    const apps = Array.isArray(appsRaw) ? (appsRaw as unknown[]) : [];
    const items = apps
      .filter((entry): entry is Record<string, unknown> => Boolean(entry && typeof entry === "object" && !Array.isArray(entry)))
      .map((entry) => {
        ...
        const enabled = entry.enabled === undefined ? true : entry.enabled === true || entry.enabled === "true";
        const mode = entry.mode === "app" ? "app" : "ai-bot";
        return { id, enabled, running: false, mode };
      })
      .filter((item) => item.id === workspaceIdentityId);
    return jsonResponse({ ok: true, items });
  });

  addRoute(routes, "POST", "/workspace/:id/owpenbot/identities/wecom", "client", async (ctx) => {
    ensureWritable(config);
    requireClientScope(ctx, "collaborator");
    const workspace = await resolveWorkspace(config, ctx.params.id);
    const body = await readJsonBody(ctx.request);
    ...
    const mode = body.mode === "app" ? "app" : "ai-bot";
    const corpId = typeof body.corpId === "string" ? body.corpId.trim() : "";
    const agentId = typeof body.agentId === "string" ? body.agentId.trim() : "";
    const secret = typeof body.secret === "string" ? body.secret.trim() : "";
    const webhookPath = typeof body.webhookPath === "string" ? body.webhookPath.trim() : "";
    const enabled = body.enabled === undefined ? true : body.enabled === true || body.enabled === "true";
    const workspaceIdentityId = normalizeOwpenbotIdentityId(workspace.id);
    ...
    if (mode === "ai-bot" && (!token || !encodingAesKey)) {
      throw new ApiError(400, "token_required", "WeCom token and encodingAesKey are required for ai-bot mode");
    }
    if (mode === "app" && (!corpId || !agentId || !secret)) {
      throw new ApiError(400, "token_required", "corpId, agentId, and secret are required for app mode");
    }

    await requireApproval(ctx, {
      workspaceId: workspace.id,
      action: "owpenbot.wecom.identity.upsert",
      summary: `Upsert WeCom identity (${identityId})`,
      paths: [resolveOwpenbotConfigPath()],
    });

    await persistOwpenbotWecomIdentity({
      id: identityId,
      mode,
      ...(token ? { token } : {}),
    ...
    const response: Record<string, unknown> = {
      ...
      persisted: true,
      applied: apply.applied,
      wecom: { id: identityId, enabled, mode },
    };

    if (apply.body && typeof apply.body === "object") {
      const record = apply.body as Record<string, unknown>;
      if (record.wecom && typeof record.wecom === "object") {
        response.wecom = record.wecom;
      }
    }
    ...
    await recordAudit(workspace.path, {
      ...
      actor: ctx.actor ?? { type: "remote" },
      action: "owpenbot.wecom.identity.upsert",
      target: "owpenbot.wecom",
      summary: `Upserted WeCom identity (${identityId})`,
      timestamp: Date.now(),
    });
  ...
  addRoute(routes, "DELETE", "/workspace/:id/owpenbot/identities/wecom/:identityId", "client", async (ctx) => {
    ensureWritable(config);
    requireClientScope(ctx, "collaborator");
    const workspace = await resolveWorkspace(config, ctx.params.id);
    const workspaceIdentityId = normalizeOwpenbotIdentityId(workspace.id);
    ...
    await requireApproval(ctx, {
      workspaceId: workspace.id,
      action: "owpenbot.wecom.identity.delete",
      summary: `Delete WeCom identity (${identityId})`,
      paths: [resolveOwpenbotConfigPath()],
    });

    const deleted = await deleteOwpenbotWecomIdentity(identityId);
    const healthPortParam = parseInteger(ctx.url.searchParams.get("healthPort") ?? undefined);
    const port = healthPortParam ?? resolveOwpenbotHealthPort();
    const requestHost = ctx.url.hostname;
    const apply = await tryFetchOwpenbotHealth(
      "DELETE",
      `/identities/wecom/${encodeURIComponent(identityId)}`,
      {
        port,
        requestHost,
        timeoutMs: 3_000,
    ...
    await recordAudit(workspace.path, {
      ...
      action: "owpenbot.wecom.identity.delete",
      target: "owpenbot.wecom",
      summary: `Deleted WeCom identity (${identityId})`,
      timestamp: Date.now(),
    });
  ...
  addRoute(routes, "POST", "/workspace/:id/opencode-router/bindings", "client", async (ctx) => {
    ...
    if (channel !== "telegram" && channel !== "slack" && channel !== "wecom") {
      throw new ApiError(400, "invalid_channel", "channel must be 'telegram', 'slack', or 'wecom'");
    }
    if (!peerId) {
...
async function persistOwpenbotWecomIdentity(identity: {
  ...
  const mode = identity.mode === "app" ? "app" : "ai-bot";
  const token = identity.token?.trim() ?? "";
  ...
  const webhookPath = identity.webhookPath?.trim() ?? "";
  const directory = typeof identity.directory === "string" ? identity.directory.trim() : "";
  if (mode === "ai-bot" && (!token || !encodingAesKey)) {
    throw new ApiError(400, "token_required", "WeCom token and encodingAesKey are required for ai-bot mode");
  }
  ...
  for (const entry of apps) {
    ...
    found = true;
    const prevDir = typeof record.directory === "string" ? record.directory.trim() : "";
    const prevMode = record.mode === "app" ? "app" : "ai-bot";
    ...
    const nextDir = directory || prevDir;
    const nextMode = identity.mode ? mode : prevMode;
...
async function deleteOwpenbotWecomIdentity(idRaw: string): Promise<boolean> {
