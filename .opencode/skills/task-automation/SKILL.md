...
本技能支持与 Task Center UI 集成：

- **使用 Task Center 向导**：在 Task Center 中点击"生成计划"，向导会自动分析需求、识别仓库，然后调用本技能生成完整计划。
- **直接调用**：也可以通过 OpenCode Chat 直接调用本技能处理工作项。
...
**如果在 Task Center UI 中已完成需求分析：**
Task Center 的需求智能分析器会自动：
1. 解析 TFS 工作项内容
2. 识别应修改的代码仓库
3. 提取关键词和技术栈

此时可以直接从 **Phase 2: 设计生成** 开始，无需重复分析。
